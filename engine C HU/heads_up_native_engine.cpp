...
constexpr int COMPACT_CONTEXT_FEATURES = 13;
constexpr int COMPACT_CARD_SLOTS = 7;
constexpr int COMPACT_HISTORY_FEATURES = 7;
constexpr int COMPACT_ACTION_FEATURES = 2;
constexpr int COMPACT_HISTORY_OFFSET = COMPACT_CONTEXT_FEATURES + COMPACT_CARD_SLOTS;

static int compact_width(int max_history) {
    return COMPACT_HISTORY_OFFSET
        + max_history * COMPACT_HISTORY_FEATURES
        + ACTIONS * COMPACT_ACTION_FEATURES;
}

static int compact_semantic_id(const ActionRecord& event) {
    const std::string kind = normalized_kind(event);
    if (kind == "fold") return 1;
    if (kind == "check") return 2;
    if (kind == "call") return 3;
    if (kind == "bet") return 4;
    if (kind == "raise") return 5;
    throw std::runtime_error("unsupported compact semantic action");
}

static py::array_t<float> encode_compact_information_state_native(
    const HeadsUpState& state,
    int hero,
    const std::vector<int>& legal_actions,
    double big_blind,
    ...
    if (!legal_actions.empty() && state.to_act != hero) {
        throw py::value_error(
            "live compact encoding requires hero == state.to_act"
        );
    }
    if (state.history_size > max_history) {
        throw py::value_error(
            "compact information-state history exceeds the lossless capacity: "
            + std::to_string(state.history_size)
            + " > "
            + std::to_string(max_history)
        );
    }
    if (action_descriptors_object.is_none() && !legal_actions.empty()) {
        throw py::value_error(
            "live compact encoding requires exact action descriptors"
        );
    }

    const int opponent = 1 - hero;
    const double total_initial = static_cast<double>(
        state.initial_stacks[0] + state.initial_stacks[1]
    );
    if (!(total_initial > 0.0)) {
        throw py::value_error("total initial chips must be positive");
    }
    const int expected = compact_width(max_history);
    std::vector<float> values;
    values.reserve(static_cast<std::size_t>(expected));
    const int last_raiser_id = state.last_full_raiser < 0
        ? 0
        : state.last_full_raiser == hero ? 1 : 2;
    values.push_back(static_cast<float>(state.street));
    values.push_back(state.button == hero ? 1.0f : 0.0f);
    values.push_back(static_cast<float>(last_raiser_id));
    values.push_back(state.raise_rights[hero] ? 1.0f : 0.0f);
    values.push_back(state.raise_rights[opponent] ? 1.0f : 0.0f);
    values.push_back(static_cast<float>(
        std::log1p(total_initial / big_blind) / std::log1p(200.0)
    ));
    values.push_back(static_cast<float>(state.initial_stacks[hero] / total_initial));
    values.push_back(static_cast<float>(state.stacks[hero] / total_initial));
    values.push_back(static_cast<float>(state.stacks[opponent] / total_initial));
    values.push_back(static_cast<float>(state.street_contrib[hero] / total_initial));
    values.push_back(static_cast<float>(state.street_contrib[opponent] / total_initial));
    values.push_back(static_cast<float>(state.min_raise / total_initial));
    values.push_back(static_cast<float>(
        static_cast<double>(state.small_blind) / big_blind
    ));

    if (state.hole[hero].size() != 2) {
        throw py::value_error("hero must have exactly two hole cards");
    }
    std::array<int, 2> hero_cards{{state.hole[hero][0], state.hole[hero][1]}};
    std::sort(hero_cards.begin(), hero_cards.end());
    for (int card : hero_cards) values.push_back(static_cast<float>(card + 1));
    std::vector<int> board = state.board.vector();
    if (board.size() >= 3) {
        std::sort(board.begin(), board.begin() + 3);
    }
    for (int index = 0; index < 5; ++index) {
        values.push_back(
            index < static_cast<int>(board.size())
            ? static_cast<float>(board[static_cast<std::size_t>(index)] + 1)
            : 0.0f
        );
    }

    const auto history = history_vector(state);
    for (const auto& event : history) {
        values.push_back(static_cast<float>(event.street + 1));
        values.push_back(event.player == hero ? 1.0f : 2.0f);
        values.push_back(static_cast<float>(compact_semantic_id(event)));
        values.push_back(static_cast<float>(event.contribution_after / total_initial));
        values.push_back(static_cast<float>(event.pot_before / total_initial));
        values.push_back(event.all_in ? 1.0f : 0.0f);
        values.push_back(event.full_raise ? 1.0f : 0.0f);
    }
    values.insert(
        values.end(),
        static_cast<std::size_t>(
            (max_history - state.history_size) * COMPACT_HISTORY_FEATURES
        ),
        0.0f
    );
    ...
    for (int action = 0; action < ACTIONS; ++action) {
        ...
        if (legal[action] != supplied) {
            throw py::value_error(
                legal[action]
                ? "missing descriptor for legal action " + std::to_string(action)
                : "descriptor supplied for illegal action " + std::to_string(action)
            );
        }
        if (!supplied) {
        ...
        const double target = descriptor_field(
            descriptors[action], "target"
        ).cast<double>();
        values.push_back(1.0f);
        values.push_back(static_cast<float>(target / total_initial));
    }

    if (static_cast<int>(values.size()) != expected) {
        throw std::runtime_error(
            "native compact encoder produced "
            + std::to_string(values.size())
            + " values; expected "
            + std::to_string(expected)
        );
...
PYBIND11_MODULE(heads_up_native_engine, module) {
    ...
    py::class_<HeadsUpHoldemEngine>(module, "HeadsUpHoldemEngine")
        ...
        .def(
            ...
            [](HeadsUpHoldemEngine& engine,
               ...
               int max_history) {
                ...
                const int width = PUBLIC_PREFIX_FEATURES
                    + CARD_TOKEN_COUNT * CARD_FEATURES
                    + max_history * HISTORY_FEATURES
                    + ACTIONS
                    + ACTIONS * ACTION_DESCRIPTOR_FEATURES;
                ...
                py::array_t<float> masks(
                    std::vector<py::ssize_t>{
                        static_cast<py::ssize_t>(states.size()),
                        static_cast<py::ssize_t>(ACTIONS)
                    }
                );
                auto output = observations.mutable_unchecked<2>();
                auto legal_masks = masks.mutable_unchecked<2>();
                for (std::size_t row = 0; row < states.size(); ++row) {
                    const auto& state = states[row];
                    if (state.terminal || state.to_act < 0) {
                        throw py::value_error(
                            "batch encoder requires live decision states"
                        );
                    }
                    const auto legal = engine.legal_actions(state);
                    const auto descriptors = engine.action_descriptors(state);
                    const auto encoded = encode_information_state_native(
                        state,
                        state.to_act,
                        legal,
                        static_cast<double>(engine.big_blind),
                        max_history,
                        descriptors
                    );
                    const auto source = encoded.unchecked<1>();
                ...
                return py::make_tuple(observations, masks);
            },
            py::arg("states"),
        ...
        .def(
            ...
            [](HeadsUpHoldemEngine& engine,
               ...
               int max_history) {
                if (max_history <= 0) {
                    throw py::value_error("max_history must be positive");
                }
                const int width = compact_width(max_history);
                py::array_t<float> observations(
                    std::vector<py::ssize_t>{
                        static_cast<py::ssize_t>(states.size()),
                        static_cast<py::ssize_t>(width)
                ...
                for (std::size_t row = 0; row < states.size(); ++row) {
                    ...
                    if (state.terminal || state.to_act < 0) {
                        throw py::value_error(
                            "compact batch encoder requires live decision states"
                        );
                    }
                    const auto legal = engine.legal_actions(state);
                    const auto descriptors = engine.action_descriptors(state);
                    const auto encoded = encode_compact_information_state_native(
                        state,
                        state.to_act,
                        legal,
                        static_cast<double>(engine.big_blind),
                    ...
                    const auto source = encoded.unchecked<1>();
                    for (int column = 0; column < width; ++column) {
                        output(static_cast<py::ssize_t>(row), column) = source(column);
                    }
                    for (int action = 0; action < ACTIONS; ++action) {
                        legal_masks(static_cast<py::ssize_t>(row), action) = 0.0f;
                    }
                    for (int action : legal) {
                        legal_masks(static_cast<py::ssize_t>(row), action) = 1.0f;
                    }
                }
                return py::make_tuple(observations, masks);
            },
            py::arg("states"),
            py::arg("max_history") = 106
        )
    ...
    module.attr("NATIVE_ABI_VERSION") = 6;
    module.attr("DEFAULT_MAX_HISTORY") = 32;
    module.attr("COMPACT_ENCODER_SCHEMA_VERSION") =
        "hu_compact_information_state_v1_full_history";
    module.attr("COMPACT_DEFAULT_MAX_HISTORY") = 106;
    module.attr("COMPACT_INPUT_DIM") = compact_width(106);
