...
class HeadsUpModelSchemaTests(unittest.TestCase):
    ...
    def test_encoder_width_offsets_and_network_output_contract(self) -> None:
        ...
        conditioned = build_policy_network(
            ACTION_CONDITIONED_ARCHITECTURE,
            encoded.numel(),
            hidden=32,
            blocks=1,
        ...
        self.assertEqual(conditioned(encoded.unsqueeze(0)).shape, (1, NUM_ACTIONS))
        self.assertFalse(hasattr(conditioned, "range_head"))

    def test_action_conditioned_network_has_direct_descriptor_gradient(self) -> None:
        state = self.env.new_hand(button=0)
        encoded = self._encode(self.env, state).unsqueeze(0).requires_grad_(True)
        network = HeadsUpDeepCFRActionConditionedV5Network(
            encoded.shape[1], hidden=32, blocks=0
        )
        with torch.no_grad():
            network.shared_action_scorer[-1].weight.fill_(0.1)
        output = network(encoded)
        all_in = 9
        output[0, all_in].backward()
        start = action_descriptor_offset() + all_in * ACTION_DESCRIPTOR_FEATURES
        gradient = encoded.grad[0, start : start + ACTION_DESCRIPTOR_FEATURES]
