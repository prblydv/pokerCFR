...
class ReservoirBuffer:
    ...
    def street_balanced_field_batches(
        ...
        self._compact()
        assert isinstance(self.memory, _PackedReservoirView)
        observations = self.memory.fields[0]
...
class ThreePlayerNeuralCFR:
    ...
    def _traverse(
        ...
        self.policy_buffers[player].add(
            (
                x.cpu().to(torch.float16),
                probabilities.cpu(),
                mask.cpu(),
    ...
    def _buffer_weight_mean(buffer: ReservoirBuffer) -> float:
        ...
        if isinstance(buffer.memory, _PackedReservoirView):
            ...
            return max(
                1e-8,
                float(buffer.memory.fields[3].mean(dtype=torch.float64).item()),
            )
        return max(
            1e-8,
    ...
    def _fit_advantage(self, player: int, steps: int, batch_size: int) -> float:
        ...
        weight_scale = self._buffer_weight_mean(self.advantage_buffers[player])
        if reinitialize:
        ...
        if self.network_architecture == "deep_cfr_branch_v2":
            batches = self.advantage_buffers[player].street_balanced_field_batches(
                batch_size, steps
            )
        elif reinitialize:
            batches = self.advantage_buffers[player].shuffled_field_batches(
                batch_size, steps
            )
        else:
            batches = (
                self.advantage_buffers[player].sample_fields(batch_size)
    ...
    def _fit_policy(self, player: int, steps: int, batch_size: int) -> float:
        ...
        weight_scale = self._buffer_weight_mean(self.policy_buffers[player])
        net.train()
        loss_sum = torch.zeros((), device=self.device)
        batches = (
            self.policy_buffers[player].street_balanced_field_batches(
                batch_size, steps
            )
            if self.network_architecture == "deep_cfr_branch_v2"
            else self.policy_buffers[player].shuffled_field_batches(
                batch_size, steps
            )
        )
        for xs, targets, masks, weights in batches:
...
__all__ = ["ReservoirBuffer", "ThreePlayerNeuralCFR"]
