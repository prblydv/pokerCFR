...
class ProductionWorkflowTests(unittest.TestCase):
    ...
    def test_policy_snapshot_and_paired_evaluation(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            argmax = evaluate_against_profile(
                self.trainer,
                "random",
                games_per_player=3,
                seed=123,
            ...
            self.assertEqual(argmax.summary["hands"], 9.0)

            with self.assertRaisesRegex(ValueError, "hero_action_mode"):
                evaluate_against_profile(
                    self.trainer,
                    "random",
                    games_per_player=1,
                    hero_action_mode="invalid",
                )
    ...
    def test_campaign_autosave_resume_and_continue(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            previous_learning_rate = resumed.learning_rate
            ...
            self.assertEqual(
                history_rows[0]["trainer_changes"]["learning_rate"],
                {"previous": previous_learning_rate, "current": 1e-3},
            )
            self.assertEqual(
