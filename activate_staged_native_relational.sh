...
STAGE="$ROOT/.staged_native_relational_20260722"
BACKUP="$ROOT/.backup_before_native_relational_20260722"

if [[ ! -f "$STAGE/STAGING_VALIDATED" ]]; then
  echo "Refusing to activate: staged relational build has not passed validation." >&2
  exit 1
fi
...
echo "Native relational features activated. Restart the notebook kernel, then resume."
echo "Backup: $BACKUP"
