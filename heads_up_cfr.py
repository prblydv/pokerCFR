...
def adjust_oversized_all_in_regret(
    action_values: torch.Tensor,
    raw_regrets: torch.Tensor,
    legal_mask: torch.Tensor,
    *,
    risk_ratio: float,
    threshold: float = 2.0,
    superiority_margin_bb: float = 0.25,
) -> tuple[torch.Tensor, float]:
    """Shrink only marginal positive regret for an oversized voluntary shove."""

    adjusted = raw_regrets.clone()
    if (
        not math.isfinite(float(risk_ratio))
        or float(risk_ratio) <= float(threshold)
        or float(adjusted[ACTION_ALL_IN]) <= 0.0
        or float(legal_mask[ACTION_ALL_IN]) <= 0.0
    ):
        return adjusted, 1.0
    legal_non_all_in = (legal_mask > 0).clone()
    legal_non_all_in[ACTION_ALL_IN] = False
    if not bool(legal_non_all_in.any()):
        return adjusted, 1.0
    best_non_all_in = float(action_values[legal_non_all_in].max().item())
    all_in_edge = float(action_values[ACTION_ALL_IN].item()) - best_non_all_in
    required_edge = float(superiority_margin_bb) * max(
        1.0, float(risk_ratio) / float(threshold)
    )
    evidence = max(0.0, min(1.0, all_in_edge / max(required_edge, 1e-8)))
    adjusted[ACTION_ALL_IN] *= evidence
...
def _parallel_traversal_worker(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "oversized_all_in_nodes": worker._oversized_all_in_nodes,
        "adjusted_all_in_nodes": worker._adjusted_all_in_nodes,
        "all_in_risk_ratios": worker._all_in_risk_ratios,
    }
...
class HeadsUpNeuralCFR:
    ...
    def __init__(
        ...
        *,
        ...
        policy_network_architecture: str = POLICY_NETWORK_ARCHITECTURE,
        encoder_schema_version: str = ENCODER_SCHEMA_VERSION,
        enable_range_training: bool = True,
        risk_aware_all_in: bool = False,
        all_in_risk_threshold: float = 2.0,
        all_in_superiority_margin_bb: float = 0.25,
        robust_advantage_loss: bool = False,
        fit_reservoir_once_per_iteration: bool = False,
        seed: int = 42,
    ) -> None:
        self.env = env
        self.device = torch.device(device)
        self.network_architecture = str(network_architecture)
        self.policy_network_architecture = str(policy_network_architecture)
        self.encoder_schema_version = str(encoder_schema_version)
        self.range_training_enabled = bool(enable_range_training)
        self.risk_aware_all_in = bool(risk_aware_all_in)
        self.all_in_risk_threshold = float(all_in_risk_threshold)
        self.all_in_superiority_margin_bb = float(all_in_superiority_margin_bb)
        self.robust_advantage_loss = bool(robust_advantage_loss)
        self.fit_reservoir_once_per_iteration = bool(
            fit_reservoir_once_per_iteration
        )
        ...
        self.range_capacity = int(range_capacity)
        ...
        if (
            not math.isfinite(self.all_in_risk_threshold)
            or self.all_in_risk_threshold <= 0.0
        ):
            raise ValueError("all_in_risk_threshold must be finite and positive")
        if (
            not math.isfinite(self.all_in_superiority_margin_bb)
            or self.all_in_superiority_margin_bb <= 0.0
        ):
            raise ValueError(
                "all_in_superiority_margin_bb must be finite and positive"
            )
        if self.network_architecture not in NETWORK_ARCHITECTURES:
            raise ValueError(
                f"unknown advantage architecture: {self.network_architecture!r}"
            )
        if self.policy_network_architecture not in NETWORK_ARCHITECTURES:
            raise ValueError(
                f"unknown policy architecture: {self.policy_network_architecture!r}"
            )
        compact_encoder = (
            self.encoder_schema_version == COMPACT_ENCODER_SCHEMA_VERSION
        )
        if self.encoder_schema_version not in (
            ENCODER_SCHEMA_VERSION,
            COMPACT_ENCODER_SCHEMA_VERSION,
        ):
            raise ValueError(
                f"unknown HU encoder schema {self.encoder_schema_version!r}"
            )
        ...
        selected_architectures = {
            ...
            self.policy_network_architecture,
        }
        if compact_encoder != bool(selected_architectures & compact_architectures):
            raise ValueError(
                "compact V6 networks and the compact full-history encoder must "
                "be selected together"
            )
        if compact_encoder and not selected_architectures <= compact_architectures:
            raise ValueError("compact encoder cannot be mixed with legacy networks")
        if self.range_training_enabled:
            expected_range_architecture = (
                COMPACT_V6_POLICY_RANGE_ARCHITECTURE
                if compact_encoder
                else POLICY_RANGE_AUX_ARCHITECTURE
            )
            if self.policy_network_architecture != expected_range_architecture:
                raise ValueError(
                    "range training requires the opponent-range policy architecture"
                )
            if self.range_capacity <= 0:
                raise ValueError("range_capacity must be positive")
        elif self.range_loss_weight != 0.0:
            raise ValueError(
                "range_loss_weight must be zero when range training is disabled"
            )
        if (
        ...
        if compact_encoder:
            self.encoder = compact_encoder_metadata(self.max_history)
            self.input_dim = compact_information_state_size(self.max_history)
        else:
        ...
        self._oversized_all_in_nodes = 0
        self._adjusted_all_in_nodes = 0
        self._all_in_risk_ratios: list[float] = []
        self._last_fit_batches = {"advantage": [0, 0], "policy": [0, 0]}
    ...
    def _risk_adjusted_regrets(
        ...
        if not self.risk_aware_all_in or float(mask[ACTION_ALL_IN]) <= 0.0:
            return raw_regrets
        descriptor_start = action_descriptor_offset(self.max_history)
        ratio_index = ACTION_DESCRIPTOR_FEATURE_NAMES.index(
            "payment_over_pot_after_call"
        )
        risk_ratio = float(
            x[
                descriptor_start
                + ACTION_ALL_IN * len(ACTION_DESCRIPTOR_FEATURE_NAMES)
                + ratio_index
            ].item()
        )
        if risk_ratio > self.all_in_risk_threshold:
            self._oversized_all_in_nodes += 1
            self._all_in_risk_ratios.append(risk_ratio)
        adjusted, evidence = adjust_oversized_all_in_regret(
            action_values,
            raw_regrets,
            mask,
            risk_ratio=risk_ratio,
            threshold=self.all_in_risk_threshold,
            superiority_margin_bb=self.all_in_superiority_margin_bb,
        )
        if evidence < 1.0:
            self._adjusted_all_in_nodes += 1
        return adjusted
    ...
    def encode(
        ...
        encoder = (
            encode_compact_information_state
            if self.encoder_schema_version == COMPACT_ENCODER_SCHEMA_VERSION
            else encode_information_state
        )
        return encoder(
            state,
            int(player),
    ...
    def add_range_training_samples(
        ...
        if not self.range_training_enabled:
            raise RuntimeError("opponent-range training is disabled")
    ...
    def _traverse(self, state, traverser: int, depth: int = 0) -> float:
        ...
        if actor == traverser:
            ...
            raw_regrets = (action_values - node_value) * mask
            regrets = self._risk_adjusted_regrets(
                x, action_values, raw_regrets, mask
            )
    ...
    def _parallel_trainer_config(self) -> dict[str, Any]:
        return {
            ...
            "advantage_reinitialize_cycle": self.advantage_reinitialize_cycle,
            "range_loss_weight": self.range_loss_weight,
            "range_capacity": max(1, self.range_capacity),
            "network_architecture": self.network_architecture,
            "policy_network_architecture": self.policy_network_architecture,
            "encoder_schema_version": self.encoder_schema_version,
            "enable_range_training": self.range_training_enabled,
            "risk_aware_all_in": self.risk_aware_all_in,
            "all_in_risk_threshold": self.all_in_risk_threshold,
            "all_in_superiority_margin_bb": self.all_in_superiority_margin_bb,
            "robust_advantage_loss": self.robust_advantage_loss,
            "fit_reservoir_once_per_iteration": (
                self.fit_reservoir_once_per_iteration
            ),
    ...
    def _collect_parallel_traversals(
        ...
        for result in results:
            ...
            self._oversized_all_in_nodes += int(
                result["oversized_all_in_nodes"]
            )
            self._adjusted_all_in_nodes += int(
                result["adjusted_all_in_nodes"]
            )
            self._all_in_risk_ratios.extend(result["all_in_risk_ratios"])
        return worker_count
    ...
    def _fit_advantage(
        ...
        *,
        ...
        if self.fit_reservoir_once_per_iteration:
            steps = math.ceil(len(buffer) / max(1, int(batch_size)))
        elif fresh_fit:
        ...
        batches = (
            ...
            if fresh_fit or self.fit_reservoir_once_per_iteration
            else (
        ...
        for xs, targets, masks, weights in batches:
            ...
            if self.robust_advantage_loss:
                ...
                elementwise = ((prediction - targets) * masks).square()
        ...
        network.eval()
        self._last_fit_batches["advantage"][player] = int(steps)
    ...
    def _fit_policy(
        ...
        *,
        ...
        range_buffer = (
            self.range_buffers[player] if self.range_training_enabled else None
        )
        has_range_data = bool(range_buffer is not None and range_buffer.memory)
        ...
        network.eval()
        self._last_fit_batches["policy"][player] = int(steps)
        return (
    ...
    def train_iteration(
        ...
        *,
        ...
        row: dict[str, float] = {
            ...
            "oversized_all_in_nodes": float(self._oversized_all_in_nodes),
            "adjusted_all_in_regret_nodes": float(self._adjusted_all_in_nodes),
            "mean_oversized_all_in_risk_ratio": (
                float(sum(self._all_in_risk_ratios) / len(self._all_in_risk_ratios))
                if self._all_in_risk_ratios
                else float("nan")
            ),
            ...
            "range_training_enabled": float(self.range_training_enabled),
            "parallel_player_fitting": float(self.device.type == "cuda"),
        }
        for player in range(NUM_PLAYERS):
            row[f"advantage_fit_batches_p{player}"] = float(
                self._last_fit_batches["advantage"][player]
            )
            row[f"policy_fit_batches_p{player}"] = float(
                self._last_fit_batches["policy"][player]
            )
    ...
    def save(self, path: str | Path, include_buffers: bool = True) -> Path:
        ...
        checkpoint: dict[str, Any] = {
            ...
            "network_architecture": self.network_architecture,
            "policy_network_architecture": self.policy_network_architecture,
            "range_training_enabled": self.range_training_enabled,
            ...
            "config": {
                ...
                "risk_aware_all_in": self.risk_aware_all_in,
                "all_in_risk_threshold": self.all_in_risk_threshold,
                "all_in_superiority_margin_bb": (
                    self.all_in_superiority_margin_bb
                ),
                "robust_advantage_loss": self.robust_advantage_loss,
                "fit_reservoir_once_per_iteration": (
                    self.fit_reservoir_once_per_iteration
    ...
    def load(
        ...
        *,
        ...
        stored_advantage_architecture = str(
            checkpoint.get("network_architecture", "")
        )
        stored_policy_architecture = str(
            checkpoint.get("policy_network_architecture", "")
        )
        if stored_advantage_architecture not in NETWORK_ARCHITECTURES:
            raise ValueError("checkpoint advantage architecture is unsupported")
        if stored_policy_architecture not in NETWORK_ARCHITECTURES:
            raise ValueError("checkpoint policy architecture is unsupported")
        range_training_enabled = bool(
            checkpoint.get(
                "range_training_enabled",
                stored_policy_architecture in (
                    POLICY_RANGE_AUX_ARCHITECTURE,
                    COMPACT_V6_POLICY_RANGE_ARCHITECTURE,
                ),
            )
        )
        if range_training_enabled and (
        ...
        config.setdefault("network_architecture", stored_advantage_architecture)
        config.setdefault("policy_network_architecture", stored_policy_architecture)
        config.setdefault("encoder_schema_version", ENCODER_SCHEMA_VERSION)
        config.setdefault("enable_range_training", range_training_enabled)
        expected_encoder = (
            compact_encoder_metadata(int(config["max_history"]))
            if config["encoder_schema_version"] == COMPACT_ENCODER_SCHEMA_VERSION
            else encoder_metadata(int(config["max_history"]))
        )
        ...
        if trainer.range_training_enabled and not has_independent_range_buffers:
        ...
        if "policy_buffers" in checkpoint:
            for buffer, state in zip(
                ...
                buffer.load_state_dict(state)
        if trainer.range_training_enabled and "range_buffers" in checkpoint:
            for buffer, state in zip(
