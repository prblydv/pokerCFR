...
def valid_combo_mask_from_encoded(
    ...
    if is_compact_information_state_tensor(information_states):
        exact_cards = compact_visible_card_ids(information_states)
        present = exact_cards >= 0
        exact_cards = exact_cards.clamp(min=0)
    else:
    ...
    blocked_counts = torch.zeros(
        batch,
        NUM_CARDS,
        dtype=torch.int16,
        device=information_states.device,
    )
    blocked_counts.scatter_add_(1, exact_cards, present.to(torch.int16))
    blocked = blocked_counts > 0
