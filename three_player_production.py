...
def evaluate_against_profile(
    ...
    *,
    ...
    if hero_action_mode not in {"sample", "argmax"}:
        raise ValueError("hero_action_mode must be 'sample' or 'argmax'")
    if isinstance(profile, str):
        if profile not in OPPONENT_PROFILES:
            raise ValueError(f"unknown opponent profile {profile!r}")
        opponent = OPPONENT_PROFILES[profile]()
            ...
            for index in live:
                ...
                if player == hero:
                    ...
                    action = (
                        ...
                        if hero_action_mode == "argmax"
                        else _draw_action(probabilities, action_rng)
                    )
                else:
