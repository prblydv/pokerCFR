...
snapshot. Repeated ``--policy-secondary`` options form a weighted action-
probability ensemble; provide matching values through ``--policy-weights``.
Without ``--policy``, both seats are controlled manually.
"""
...
class WeightedHeadsUpSnapshotPolicy:
    """Weighted probability ensemble of compatible HU snapshots."""

    def __init__(
        self,
        policies: list[HeadsUpSnapshotPolicy],
        weights: list[float] | tuple[float, ...] | None = None,
    ) -> None:
        if len(policies) < 2:
            raise ValueError("a weighted policy requires at least two snapshots")
        if len({policy.sha256 for policy in policies}) != len(policies):
            raise ValueError("weighted policy snapshots must be distinct")
        first = policies[0]
        compatibility_keys = (
            "model_id",
            "input_dim",
        ...
        for policy in policies[1:]:
            for key in compatibility_keys:
                if policy.snapshot.metadata.get(key) != first.snapshot.metadata.get(
                    ...
                    raise ValueError(
                        "cannot combine incompatible policy snapshots: "
                        f"metadata {key!r} differs"
                    )
            if policy.mode != first.mode:
                raise ValueError("weighted policies must use the same policy mode")
            if policy.device != first.device:
                raise ValueError("weighted policies must use the same device")
        ...
        self.iteration_label = "+".join(
            f"{iteration}@{100.0 * weight:.1f}%"
            for iteration, weight in zip(self.iterations, self.weights)
        )
        self.paths = tuple(policy.path for policy in policies)
        self.path = " + ".join(str(path) for path in self.paths)
        fingerprint_parts = [
            f"{policy.sha256}:{weight:.12f}"
            for policy, weight in zip(self.policies, self.weights)
        ]
        fingerprint_input = "weighted_probabilities_v1\0" + "\0".join(
            fingerprint_parts
        )
    ...
    def _average(self, probability_tensors) -> torch.Tensor:
        stacked = torch.stack(tuple(probability_tensors), dim=0)
        weights = torch.as_tensor(
            self.weights,
            dtype=stacked.dtype,
            device=stacked.device,
        )
        shape = (len(self.weights),) + (1,) * (stacked.ndim - 1)
        weighted = (stacked * weights.view(shape)).sum(dim=0)
        denominator = weighted.sum(dim=-1, keepdim=True)
        if torch.any(denominator <= 0.0):
            raise RuntimeError("weighted policy produced zero probability mass")
    ...
    def choose_action(self, env, state) -> tuple[int, torch.Tensor]:
        ...
        if action not in env.legal_actions(state):
            raise RuntimeError("weighted policy selected an illegal action")
        return action, probabilities
    ...
    def opponent_range(self, env, state) -> PublicRangeSnapshot:
        ...
        combined = {}
        for snapshot, component_weight in zip(snapshots, self.weights):
            for combo, range_weight in zip(snapshot.combos, snapshot.weights):
                combined[combo] = combined.get(combo, 0.0) + (
                    float(component_weight) * float(range_weight)
                )
        total = sum(combined.values())
        if total <= 0.0:
            raise RuntimeError("weighted policy produced an empty opponent range")
...
class HeadsUpManualGUI:
            ...
            "controller_configuration": {
                ...
                "policy_ensemble": {
                    ...
                    "component_weights": list(
                        getattr(self.policy, "weights", (1.0,))
                    ),
                    "component_sha256": [
                        component.sha256
    ...
    def _controller_mode(self) -> str:
        if self.policy is None:
            return "manual"
        ensemble = (
            f"+weighted_{len(self.policy.policies)}_policies"
            if isinstance(self.policy, WeightedHeadsUpSnapshotPolicy)
            else ""
        )
        if not self.search_enabled:
                    ...
                    "component_policy_probabilities": [
                        {
                            ...
                            "ensemble_weight": float(component_weight),
                            "probabilities": {
                                ACTION_NAMES[value]: float(values[value])
                                for value in legal
                            },
                        }
                        for component, component_weight, values in zip(
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=[],
        help=(
            "additional compatible HU snapshot; repeat this option to build "
            "a multi-policy probability ensemble"
        ),
    )
    parser.add_argument(
        "--policy-weights",
        ...
        default=None,
        help=(
            "weights in model order: --policy first, followed by each "
            "--policy-secondary; values are normalized automatically"
        ),
    )
    parser.add_argument(
        "--human-seat",
    ...
    if args.policy_weights is not None:
        expected = 1 + len(args.policy_secondary)
        if args.policy is None:
            parser.error("--policy-weights requires --policy")
        if len(args.policy_weights) != expected:
            parser.error(
                f"--policy-weights requires exactly {expected} values: "
                "one for --policy and one for each --policy-secondary"
            )
        if any(not math.isfinite(weight) for weight in args.policy_weights):
            parser.error("--policy-weights must be finite")
        if any(weight < 0.0 for weight in args.policy_weights):
            parser.error("--policy-weights must be nonnegative")
        if sum(args.policy_weights) <= 0.0:
            parser.error("--policy-weights must contain a positive value")
    if args.policy_weights is not None and not args.policy_secondary:
        parser.error("--policy-weights is only needed for a multi-policy ensemble")
    if args.policy_secondary and not args.no_search:
        parser.error("multi-policy ensembles currently require --no-search")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.policy_secondary:
            policy = WeightedHeadsUpSnapshotPolicy(
                [
                    primary_policy,
                    *[
                        HeadsUpSnapshotPolicy(
