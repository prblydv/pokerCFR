...
class NativeHeadsUpEngineTests(unittest.TestCase):
    ...
    def test_native_binary_schema_handshake_matches_python_contract(self):
        ...
        self.assertIn(extension.NATIVE_ABI_VERSION, (5, 6))
        ...
        if extension.NATIVE_ABI_VERSION >= 6:
            self.assertEqual(extension.COMPACT_INPUT_DIM, 782)
            self.assertEqual(extension.COMPACT_DEFAULT_MAX_HISTORY, 106)
    ...
    def test_native_batch_encoder_matches_scalar_encoder_and_masks(self):
        states = [
            self.env.new_hand(button=button, deck=list(range(52)))
            for button in (0, 1, 0, 1)
        ]
        observations, masks = native_engine.encode_information_states_native(
            self.env,
            states,
            32,
        )
        self.assertEqual(
            observations.shape,
            (len(states), information_state_size()),
        )
        self.assertEqual(masks.shape, (len(states), native_engine.NUM_ACTIONS))
        for index, state in enumerate(states):
            legal = self.env.legal_actions(state)
            descriptors = self.env.action_descriptors(state)
            scalar = native_engine.encode_information_state_native(
                state,
                state.to_act,
                legal,
                self.env.big_blind,
            ...
            np.testing.assert_array_equal(observations[index], scalar)
            expected_mask = np.zeros(native_engine.NUM_ACTIONS, dtype=np.float32)
            expected_mask[legal] = 1.0
            np.testing.assert_array_equal(masks[index], expected_mask)
