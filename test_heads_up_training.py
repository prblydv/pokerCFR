...
from heads_up_cfr import (
    ...
    HeadsUpNeuralCFR,
    ReservoirBuffer,
    adjust_oversized_all_in_regret,
)
...
class HeadsUpTrainingTests(unittest.TestCase):
    def test_action_conditioned_campaign_has_no_range_head_or_reservoir(self):
        ...
        self.assertFalse(trainer.range_training_enabled)
        self.assertEqual(trainer.range_buffers, [])
        self.assertTrue(
            all(not hasattr(network, "range_head") for network in trainer.policy_nets)
        )
        ...
        self.assertEqual(row["range_samples"], 0.0)
        self.assertEqual(row["range_training_enabled"], 0.0)

        with tempfile.TemporaryDirectory() as directory:
            checkpoint = Path(directory) / "fresh.pt"
            ...
            self.assertFalse(restored.range_training_enabled)
            self.assertEqual(restored.range_buffers, [])
            save_policy_snapshot(restored, snapshot)
            loaded = load_policy_snapshot(snapshot)
            self.assertFalse(loaded.metadata["has_range_head"])
            self.assertEqual(
                loaded.metadata["policy_network_architecture"],
                ACTION_CONDITIONED_ARCHITECTURE,
            )
    ...
    def test_advantage_can_reinitialize_every_iteration_from_25(self):
        trainer = _small_trainer(
            reinitialize_advantage_each_iteration=True,
            advantage_reinitialize_from_iteration=25,
        ...
        trainer.iteration = 25
        trainer._fit_advantage(0, steps=1, batch_size=1)
        at_25 = trainer.advantage_nets[0]
        self.assertIsNot(at_25, original)
        trainer.iteration = 26
        trainer._fit_advantage(0, steps=1, batch_size=1)
        self.assertIsNot(trainer.advantage_nets[0], at_25)

    def test_risk_guard_preserves_negative_and_superior_all_in_regret(self):
        mask = torch.ones(NUM_ACTIONS)
        values = torch.zeros(NUM_ACTIONS)
        regrets = torch.zeros(NUM_ACTIONS)
        regrets[ACTION_ALL_IN] = -3.0
        adjusted, evidence = adjust_oversized_all_in_regret(
            values, regrets, mask, risk_ratio=20.0
        )
        self.assertEqual(float(adjusted[ACTION_ALL_IN]), -3.0)
        self.assertEqual(evidence, 1.0)

        regrets[ACTION_ALL_IN] = 2.0
        values[ACTION_ALL_IN] = 10.0
        adjusted, evidence = adjust_oversized_all_in_regret(
            values, regrets, mask, risk_ratio=20.0
        )
        self.assertEqual(float(adjusted[ACTION_ALL_IN]), 2.0)
        self.assertEqual(evidence, 1.0)

    def test_risk_guard_suppresses_only_marginal_positive_shove(self):
        mask = torch.ones(NUM_ACTIONS)
        values = torch.zeros(NUM_ACTIONS)
        values[2] = 1.0
        values[ACTION_ALL_IN] = 1.1
        regrets = torch.zeros(NUM_ACTIONS)
        regrets[ACTION_ALL_IN] = 0.5
        adjusted, evidence = adjust_oversized_all_in_regret(
            values, regrets, mask, risk_ratio=20.0
        )
        self.assertGreater(evidence, 0.0)
        self.assertLess(evidence, 1.0)
        self.assertAlmostEqual(
            float(adjusted[ACTION_ALL_IN]), 0.5 * evidence, places=6
        )

    def test_dynamic_fit_consumes_each_reservoir_row_once(self):
        trainer = _small_trainer(
        ...
        for index in range(5):
            trainer.advantage_buffers[0].add(
                (
                    torch.full(
                        (trainer.input_dim,), index, dtype=torch.float16
                    ),
                    torch.zeros(NUM_ACTIONS),
                    torch.ones(NUM_ACTIONS),
                    torch.tensor(1.0),
        ...
        trainer._fit_advantage(0, steps=100, batch_size=2)
        self.assertEqual(trainer._last_fit_batches["advantage"][0], 3)
