...
ACTION_CONDITIONED_ARCHITECTURE = "hu_deep_cfr_action_conditioned_v5"
COMPACT_V6_ARCHITECTURE = "hu_deep_cfr_compact_v6"
COMPACT_V6_POLICY_RANGE_ARCHITECTURE = "hu_deep_cfr_compact_v6_policy_range_v1"
...
class HeadsUpDeepCFRActionConditionedV5Network(nn.Module):
    """Score each candidate action against the complete information state.

    The V4 backbone remains the shared, speed-oriented card/public/history
    encoder.  Unlike the V4 linear output heads, this network gives every
    action a direct path from its exact state-dependent descriptor to a shared
    scorer.  The output contract remains ``[batch, NUM_ACTIONS]`` so Deep CFR
    traversal, raw regret targets, regret matching, and policy fitting are
    unchanged.
    """
    ...
    def __init__(self, input_dim: int, hidden: int = 128, blocks: int = 2):
        ...
        # A compact token keeps the ten-way vectorized scorer inexpensive even
        # at hidden-512.  This is an architecture width, not a poker threshold.
        self.action_token_dim = max(32, min(64, hidden // 8))
        self.context_projection = nn.Linear(hidden, self.action_token_dim)
        self.action_projection = nn.Linear(
            ACTION_DESCRIPTOR_FEATURES, self.action_token_dim
        )
        self.action_identity = nn.Embedding(NUM_ACTIONS, self.action_token_dim)
        joint_dim = 4 * self.action_token_dim
        scorer_hidden = max(32, min(128, hidden // 4))
        self.joint_norm = nn.LayerNorm(joint_dim)
        self.shared_action_scorer = nn.Sequential(
            nn.Linear(joint_dim, scorer_hidden),
            nn.SiLU(),
            nn.Linear(scorer_hidden, 1),
        )
        nn.init.normal_(self.action_identity.weight, std=0.02)
        nn.init.zeros_(self.shared_action_scorer[-1].weight)
        nn.init.zeros_(self.shared_action_scorer[-1].bias)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if x.ndim != 2 or int(x.shape[1]) != self.input_dim:
            raise ValueError(
                "HU action-conditioned V5 expects "
                f"[batch, {self.input_dim}] inputs"
            )
        batch = int(x.shape[0])
        descriptor_start = action_descriptor_offset(self.backbone.history_steps)
        descriptors = x[:, descriptor_start:].reshape(
            batch, NUM_ACTIONS, ACTION_DESCRIPTOR_FEATURES
        )
        context = F.silu(self.context_projection(self.backbone(x))).unsqueeze(1)
        context = context.expand(-1, NUM_ACTIONS, -1)
        action_ids = torch.arange(NUM_ACTIONS, device=x.device)
        actions = F.silu(
            self.action_projection(descriptors)
            + self.action_identity(action_ids).unsqueeze(0)
        )
        joint = torch.cat(
            (
                context,
                actions,
                context * actions,
                torch.abs(context - actions),
            ),
            dim=2,
        )
        return self.shared_action_scorer(self.joint_norm(joint)).squeeze(2)


def _fixed_sinusoidal_positions(length: int, width: int) -> torch.Tensor:
    """Bounded relative-recency basis used by the compact history encoder."""
...
def _compact_history_steps(input_dim: int) -> int:
    ...
    if history_values <= 0 or history_values % COMPACT_HISTORY_FEATURES:
        raise ValueError(
            f"input width {input_dim} is not a compact HU information state"
        )
    return history_values // COMPACT_HISTORY_FEATURES


class HeadsUpDeepCFRCompactV6Backbone(nn.Module):
    """Structured 40 + 7L encoder with full-history masking and stable scales."""

    def __init__(self, input_dim: int, hidden: int = 384, blocks: int = 2):
        super().__init__()
        if hidden <= 0 or blocks < 0:
            raise ValueError("hidden must be positive and blocks cannot be negative")
        self.input_dim = int(input_dim)
        self.hidden = int(hidden)
        self.history_steps = _compact_history_steps(input_dim)
        token_dim = 64
        self.token_dim = token_dim

        self.context_street = nn.Embedding(4, 24)
        self.context_last_raiser = nn.Embedding(3, 12)
        self.context_scalar_norm = nn.LayerNorm(COMPACT_CONTEXT_FEATURES - 2)
        self.context_projection = nn.Linear(
            COMPACT_CONTEXT_FEATURES - 2 + 24 + 12, hidden
        )

        self.card_rank = nn.Embedding(13, 32)
        self.card_suit = nn.Embedding(4, 16)
        self.card_role = nn.Embedding(COMPACT_CARD_SLOTS, 16)
        self.card_token_norm = nn.LayerNorm(token_dim)
        ...
        self.card_attention = nn.TransformerEncoder(
            card_layer, num_layers=1, enable_nested_tensor=False
        )
        self.card_projection = nn.Linear(4 * token_dim, hidden)
        self.poker_feature_norm = nn.LayerNorm(POKER_RELATIONAL_FEATURES)
        self.poker_feature_projection = nn.Linear(POKER_RELATIONAL_FEATURES, hidden)

        self.history_street = nn.Embedding(5, 12, padding_idx=0)
        self.history_actor = nn.Embedding(3, 10, padding_idx=0)
        self.history_action = nn.Embedding(6, 18, padding_idx=0)
        self.history_scalars = nn.Linear(4, 24)
        self.history_token_norm = nn.LayerNorm(token_dim)
        self.history_summary_token = nn.Parameter(torch.zeros(1, 1, token_dim))
        self.register_buffer(
            "history_relative_positions",
            _fixed_sinusoidal_positions(self.history_steps, token_dim),
            persistent=True,
        )
        history_layer = nn.TransformerEncoderLayer(
            d_model=token_dim,
        ...
        self.history_projection = nn.Linear(2 * token_dim, hidden)

        self.action_identity = nn.Embedding(NUM_ACTIONS, 32)
        self.action_scalars = nn.Linear(COMPACT_ACTION_FEATURES, 32)
        self.action_token_norm = nn.LayerNorm(token_dim)

        self.combine = nn.Linear(4 * hidden, hidden)
        self.trunk = nn.ModuleList(ResidualBlock(hidden) for _ in range(blocks))
        nn.init.normal_(self.history_summary_token, std=0.02)

    def _cards_as_one_hot(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ids = compact_visible_card_ids(x)
        present = ids >= 0
        safe = ids.clamp(0, 51)
        batch = int(x.shape[0])
        cards = x.new_zeros(batch, COMPACT_CARD_SLOTS, CARD_FEATURES)
        batch_index = torch.arange(batch, device=x.device).unsqueeze(1).expand_as(ids)
        slot_index = torch.arange(COMPACT_CARD_SLOTS, device=x.device).unsqueeze(0).expand_as(ids)
        cards[batch_index[present], slot_index[present], (safe % 13)[present]] = 1.0
        cards[
            batch_index[present],
            slot_index[present],
            (13 + safe // 13)[present],
        ] = 1.0
        cards[:, :, 17] = present.to(x.dtype)
        return cards, ids

    def _card_representation(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        cards, ids = self._cards_as_one_hot(x)
        present = ids >= 0
        safe = ids.clamp(0, 51)
        roles = torch.arange(COMPACT_CARD_SLOTS, device=x.device).unsqueeze(0)
        tokens = torch.cat(
            (
                self.card_rank(safe % 13),
                self.card_suit(safe // 13),
                self.card_role(roles).expand(int(x.shape[0]), -1, -1),
            ),
            dim=2,
        )
        tokens = self.card_token_norm(tokens) * present.unsqueeze(2)
        attended = self.card_attention(tokens, src_key_padding_mask=~present)
        attended = attended * present.unsqueeze(2)
        grouped = torch.cat(
            (
                attended[:, 0:2].sum(dim=1),
                attended[:, 2:5].sum(dim=1),
                attended[:, 5],
                attended[:, 6],
            ),
            dim=1,
        )
        return F.silu(self.card_projection(grouped)), cards

    def _history_representation(self, x: torch.Tensor) -> torch.Tensor:
        batch = int(x.shape[0])
        action_start = compact_action_offset(self.history_steps)
        history = x[:, COMPACT_HISTORY_OFFSET:action_start].reshape(
            batch, self.history_steps, COMPACT_HISTORY_FEATURES
        )
        present = history[:, :, 0] > 0.0
        lengths = present.sum(dim=1)
        maximum = int(lengths.max().item())
        if maximum == 0:
            return x.new_zeros(batch, self.hidden)
        history = history[:, :maximum]
        present = present[:, :maximum]
        street = history[:, :, 0].round().to(torch.long).clamp(0, 4)
        actor = history[:, :, 1].round().to(torch.long).clamp(0, 2)
        action = history[:, :, 2].round().to(torch.long).clamp(0, 5)
        scalars = history[:, :, 3:]
        tokens = torch.cat(
            (
                self.history_street(street),
                self.history_actor(actor),
                self.history_action(action),
                F.silu(self.history_scalars(scalars)),
            ),
            dim=2,
        )
        positions = torch.arange(maximum, device=x.device).unsqueeze(0)
        recency = (lengths.unsqueeze(1) - 1 - positions).clamp(
            min=0, max=self.history_steps - 1
        )
        tokens = self.history_token_norm(tokens)
        tokens = tokens + self.history_relative_positions.index_select(
            0, recency.reshape(-1)
        ).reshape(batch, maximum, self.token_dim)
        tokens = tokens * present.unsqueeze(2)
        summary = self.history_summary_token.expand(batch, -1, -1)
        ...
        memory_sequence, _ = self.history_memory(tokens)
        final_index = (lengths - 1).clamp(min=0)
        memory = memory_sequence[
            torch.arange(batch, device=x.device), final_index
        ]
        return F.silu(self.history_projection(torch.cat((attention, memory), dim=1)))

    def action_tokens(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        start = compact_action_offset(self.history_steps)
        actions = x[:, start:].reshape(
            int(x.shape[0]), NUM_ACTIONS, COMPACT_ACTION_FEATURES
        )
        legal = actions[:, :, 0]
        ids = torch.arange(NUM_ACTIONS, device=x.device).unsqueeze(0)
        tokens = torch.cat(
            (
                self.action_identity(ids).expand(int(x.shape[0]), -1, -1),
                F.silu(self.action_scalars(actions)),
            ),
            dim=2,
        )
        return self.action_token_norm(tokens), legal

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if x.ndim != 2 or int(x.shape[1]) != self.input_dim:
            raise ValueError(
                f"HU compact V6 expects [batch, {self.input_dim}] inputs"
            )
        context = x[:, :COMPACT_CONTEXT_FEATURES]
        street = compact_street_indices(x)
        last_raiser = context[:, 2].round().to(torch.long).clamp(0, 2)
        scalar = torch.cat((context[:, 1:2], context[:, 3:]), dim=1)
        context_representation = F.silu(
            self.context_projection(
                torch.cat(
                    (
                        self.context_scalar_norm(scalar),
                        self.context_street(street),
                        self.context_last_raiser(last_raiser),
                    ),
                    dim=1,
                )
            )
        )
        card_representation, cards = self._card_representation(x)
        poker_representation = F.silu(
            self.poker_feature_projection(
                self.poker_feature_norm(
                    poker_relational_features(cards, compact_street_one_hot(x))
                )
            )
        )
        history_representation = self._history_representation(x)
...
class HeadsUpDeepCFRCompactV6Network(nn.Module):
    """Hidden-state encoder with a shared exact candidate-action scorer."""

    def __init__(self, input_dim: int, hidden: int = 384, blocks: int = 2):
        super().__init__()
        self.input_dim = int(input_dim)
        self.hidden = int(hidden)
        self.backbone = HeadsUpDeepCFRCompactV6Backbone(input_dim, hidden, blocks)
        token_dim = self.backbone.token_dim
        self.context_projection = nn.Linear(hidden, token_dim)
        self.joint_norm = nn.LayerNorm(4 * token_dim)
        self.scorer = nn.Sequential(
            nn.Linear(4 * token_dim, max(64, hidden // 3)),
            nn.SiLU(),
            nn.Linear(max(64, hidden // 3), 1),
        )
        nn.init.zeros_(self.scorer[-1].weight)
        nn.init.zeros_(self.scorer[-1].bias)

    def representation_and_actions(
        self, x: torch.Tensor
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        representation = self.backbone(x)
        actions, legal = self.backbone.action_tokens(x)
        return representation, actions, legal

    def _score(
        self, representation: torch.Tensor, actions: torch.Tensor
    ) -> torch.Tensor:
        context = F.silu(self.context_projection(representation)).unsqueeze(1)
        context = context.expand_as(actions)
        joint = torch.cat(
            (context, actions, context * actions, torch.abs(context - actions)), dim=2
        )
        return self.scorer(self.joint_norm(joint)).squeeze(2)

    def _action_logits(
        self, representation: torch.Tensor, x: torch.Tensor
    ) -> torch.Tensor:
        """Score encoded candidates from an already-computed state trunk."""

        actions, _ = self.backbone.action_tokens(x)
        return self._score(representation, actions)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        representation, actions, _ = self.representation_and_actions(x)
        return self._score(representation, actions)


class HeadsUpDeepCFRCompactV6PolicyRangeNetwork(
    HeadsUpDeepCFRCompactV6Network
):
    def __init__(self, input_dim: int, hidden: int = 384, blocks: int = 2):
    ...
    def forward_with_range(
        ...
        representation, actions, _ = self.representation_and_actions(x)
        return self._score(representation, actions), self.range_head(representation)
...
def build_advantage_network(
    ...
    if architecture == ACTION_CONDITIONED_ARCHITECTURE:
        return HeadsUpDeepCFRActionConditionedV5Network(
            input_dim, hidden, blocks
        )
    if architecture == COMPACT_V6_ARCHITECTURE:
        return HeadsUpDeepCFRCompactV6Network(input_dim, hidden, blocks)
...
def build_policy_network(
    ...
    if architecture == COMPACT_V6_ARCHITECTURE:
        return HeadsUpDeepCFRCompactV6Network(input_dim, hidden, blocks)
    if architecture == COMPACT_V6_POLICY_RANGE_ARCHITECTURE:
        return HeadsUpDeepCFRCompactV6PolicyRangeNetwork(
            input_dim, hidden, blocks
        )
    raise ValueError(f"unknown heads-up network architecture: {architecture!r}")
...
__all__ = [
    ...
    "HeadsUpDeepCFRCompactV4Network",
    "HeadsUpDeepCFRActionConditionedV5Network",
