...
def save_policy_snapshot(
    ...
    *,
    ...
    payload = {
        ...
        "policy_network_architecture": trainer.policy_network_architecture,
        "has_range_head": bool(trainer.range_training_enabled),
        ...
        "encoder_schema_version": trainer.encoder_schema_version,
        "encoder_metadata": dict(trainer.encoder),
...
def load_policy_snapshot(
    ...
    *,
    ...
    if payload.get("kind") != "heads_up_policy_snapshot" or version not in (2, 3, 4, 5):
    ...
    metadata.update(
        {
            ...
            "encoder_schema_version": payload.get(
                "encoder_schema_version", ENCODER_SCHEMA_VERSION
            ),
            "encoder_metadata": dict(payload.get("encoder_metadata", {})),
            ...
            "has_range_head": bool(
                version == 3
                or (version >= 4 and payload.get("has_range_head", False))
            ),
...
def range_reservoir_statistics(
    ...
    *,
    ...
    compact = is_compact_information_state_tensor(xs)
    streets = compact_street_indices(xs) if compact else xs[:, :4].argmax(dim=1)
    ...
    if compact:
        visible_ids = compact_visible_card_ids(xs)
        cards = torch.zeros(
            (len(xs), 7, CARD_FEATURES), dtype=xs.dtype, device=xs.device
        )
        board_ids = visible_ids[:, 2:]
        board_present = board_ids >= 0
        board_exact = board_ids.clamp(min=0)
        board_rows, board_tokens = torch.nonzero(
            board_present, as_tuple=True
        )
        board_cards = board_exact[board_rows, board_tokens]
        cards[board_rows, board_tokens + 2, 17] = 1.0
        cards[board_rows, board_tokens + 2, board_cards % 13] = 1.0
        cards[board_rows, board_tokens + 2, 13 + board_cards // 13] = 1.0
        street_one_hot = compact_street_one_hot(xs)
    else:
        visible = xs[
    ...
    relational = poker_relational_features(cards, street_one_hot)
...
class ProductionCampaign:
    ...
    def __init__(
        ...
        if self.config.reference_policy_path is not None:
            ...
            if (
                ...
                raise ValueError(
                    "reference policy encoder is incompatible"
                )
    ...
    def _run_config_payload(self) -> dict[str, Any]:
        return {
            ...
            "trainer": {
                ...
                "policy_network_architecture": (
                    self.trainer.policy_network_architecture
                ),
                "range_training_enabled": self.trainer.range_training_enabled,
                ...
                "range_capacity": self.trainer.range_capacity,
            },
    ...
    def _extend_evaluation(
        ...
        """Hook for isolated campaigns with additional frozen benchmarks."""
    ...
    def _promotion_allowed(
        ...
        return float(row["benchmark_composite_lcb95_bb"]) > float(previous_lcb)
    ...
    def _evaluate(self, row: dict[str, Any]) -> None:
        ...
        if self.trainer.range_training_enabled:
            row.update(
        ...
        self._extend_evaluation(row, iteration_dir)
        ...
        promoted = self._promotion_allowed(row, previous_lcb)
        ...
        *,
        ...
        try:
            ...
            while self.trainer.iteration < self.config.target_iteration:
                ...
                if (
                    self.trainer.range_training_enabled
                    and
    ...
    *,
    ...
    if latest is not None:
        ...
        for name, buffers in capacity_requests:
            if not buffers:
                if name == "range_capacity" and trainer.range_training_enabled:
                    raise RuntimeError("range training has no range reservoirs")
                continue
            ...
            if name == "range_capacity":
                trainer.range_capacity = requested
        return trainer, True
